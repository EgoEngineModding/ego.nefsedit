...
{
	/// <summary>
	/// Takes data from an input file stream, compresses it, and writes it to the specified output file. Streams should
	/// already seek to the proper location before calling this function.
	/// </summary>
		...
		CancellationToken cancelToken)
	{
		...
		// Deflate stream doesn't write properly directly to a FileStream when doing this chunk business. So have to do
		// this multi-stream setup.
		...
		using (var deflateStream = new DeflateStream(outMemStream, CompressionMode.Compress))
		{
			...
			// Close deflate stream to finalize compression - breaking the "using()" convention, but this is needed
			deflateStream.Close();
