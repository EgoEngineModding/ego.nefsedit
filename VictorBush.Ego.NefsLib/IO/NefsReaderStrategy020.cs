...
internal class NefsReaderStrategy020 : NefsReaderStrategy
{
	private static readonly ILogger Log = NefsLog.GetLogger();

	protected override NefsVersion Version => NefsVersion.Version020;
	...
	{
		var header = await ReadTocDataAsync<NefsTocHeader020>(reader, offset, token).ConfigureAwait(false);
	...
	public override async Task<INefsHeader> ReadHeaderAsync(
		...
		NefsProgress p)
	{
		Log.LogInformation("Detected NeFS version 0.2.0.");
		...
		{
			header = await ReadTocDataAsync<NefsTocHeader020>(reader, primaryOffset, p.CancellationToken);
		}
	...
	{
		throw new InvalidOperationException("NeFS version 0.2.0 does not support separated headers.");
	}
}
