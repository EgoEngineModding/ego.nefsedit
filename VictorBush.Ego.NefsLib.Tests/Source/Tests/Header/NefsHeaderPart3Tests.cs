...
namespace VictorBush.Ego.NefsLib.Tests.Header
{
    ...
    public class NefsHeaderPart3Tests
    {
        ...
        public void NefsHeaderPart3_MultipleItems_EntriesPopulated()
        {
            ...
            Assert.Equal("file1", p3.FileNamesByOffset[0]);
            Assert.Equal("file2", p3.FileNamesByOffset[6]);
            Assert.Equal("dir1", p3.FileNamesByOffset[12]);
            Assert.Equal("file3", p3.FileNamesByOffset[17]);

            Assert.Equal(0, (int)p3.OffsetsByFileName[file1.FileName]);
            Assert.Equal(6, (int)p3.OffsetsByFileName[file2.FileName]);
            Assert.Equal(12, (int)p3.OffsetsByFileName[dir1.FileName]);
